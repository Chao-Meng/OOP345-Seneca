...
namespace sdds {
    ...
    void FoodOrder::read(istream& istr) {
        ...
        m_foodDescr = new char[vtElems[1].length()+1];
        strcpy(m_foodDescr, vtElems[1].c_str());
        m_foodPrice = stof(vtElems[2].c_str());
        string strFlag = vtElems[3];
        if (strcmp(strFlag.c_str(), "Y") == 0) {
            m_flag = true;
        }
        else {
            m_flag = false;
        ...
        if (strcmp(m_orderName,"No order")!=0) {
           if (!m_flag) {
               ...
               cout << left << setw(2) << counter++ << ". " << setw(10) << m_orderName << '|' << setw(25) <<
                ...
                cout << left << setw(2) << counter++ << ". " << setw(10) << m_orderName << '|' << setw(25) <<
                    m_foodDescr << '|' << fixed << setw(12) << setprecision(2) << m_foodPrice << '|' <<right<< fixed
                    <<setw(13)<< setprecision(2)<<m_spePrice<< endl;
           }
